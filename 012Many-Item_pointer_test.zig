...
test "Принулительное взяти [*]T" {
    var array: [10]u8 = [1]u8{1} ** 10;
    const ptr = @as([*]u8, &array);
    //Указатель на массив может быть [*]T или *[N]T
    //Первый - просто так нельзя взять
    //Нудно преобразовывать
    //А вот второй - безопасный и берется амперсантом

    MultOnTwo(ptr, array.len);
